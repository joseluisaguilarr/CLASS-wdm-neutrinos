...
int background_functions(
  ...
  int n_ncdm;
  /*JL */
  double rho_wdm,p_wdm,pseudo_p_wdm;
  double w_wdm_m, w_wdm_r, w_wdm, w_wdm_0, width_wdm, sigma_wdm;
  ...
  if (pba->has_wdm == _TRUE_) {  	  
	  ...
	  w_wdm = 1./3. * (1./(1.+ a*a/pow(pba->bdm_ac,2.)));
	  w_wdm_0 = 1./3. * (1./(1.+ pba->a_today/pow(pba->bdm_ac,2.)));
	 
  	  rho_wdm = pba->Omega0_wdm*pow(pba->H0,2.) / pow(a_rel,4) * sqrt(w_wdm_0 / w_wdm);  	    	  
	  
	  p_wdm   = w_wdm * rho_wdm;
	  /*printf("2. %e; %e ; %e ; %f \n" ,a_rel, rho_wdm, p_wdm, p_wdm/rho_wdm); */
	  pseudo_p_wdm = 3 * w_wdm* p_wdm;	

      pvecback[pba->index_bg_rho_wdm] = rho_wdm;
      rho_tot += rho_wdm;
      pvecback[pba->index_bg_p_wdm] = p_wdm;
      p_tot += p_wdm;
      pvecback[pba->index_bg_pseudo_p_wdm] = pseudo_p_wdm;

      /* (3 p_wdm) is the "relativistic" contribution to rho_wdm */
      rho_r += 3.* p_wdm;

      /* (rho_wdm - 3 p_ncdm1) is the "non-relativistic" contribution
         to rho_wdm */
      rho_m += rho_wdm - 3.* p_wdm;
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_wdm,pba->has_wdm,index_bg,1);
  class_define_index(pba->index_bg_p_wdm,pba->has_wdm,index_bg,1);
  class_define_index(pba->index_bg_pseudo_p_wdm,pba->has_wdm,index_bg,1);
...
int background_initial_conditions(
  ...
  /* JL */
  double rho_wdm, p_wdm,rho_wdm_rel_tot=0.;
...
/** 
  if (pba->has_wdm == _TRUE_) {
	...
	printf(">>> w = %g; p=%g;, rho=%g; a = %g; ac = %g \n", p_wdm/rho_wdm, p_wdm, rho_wdm, a, pba->bdm_ac );
    ...
    for (counter=0; counter < _MAX_IT_; counter++) {
      ...
      rho_wdm = pvecback[pba->index_bg_rho_wdm];
      p_wdm = pvecback[pba->index_bg_p_wdm];
      rho_wdm_rel_tot = 3.*p_wdm;      	  
	  
  	  // For some reason this is not always true, changing the if
  	  // if (fabs(p_ncdm/rho_ncdm-1./3.)>ppr->tol_ncdm_initial_w)
  	  if (a < pba->bdm_ac)
  	    is_early_enough = _TRUE_;
...
int background_output_titles(struct background * pba,
  ...
  /* JL */
  class_store_columntitle(titles,"rho_wdm",pba->has_wdm);
  class_store_columntitle(titles,"p_wdm",pba->has_wdm);
  class_store_columntitle(titles,"pseudo_p_wdm",pba->has_wdm);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    /* JL */
    class_store_double(dataptr,pvecback[pba->index_bg_rho_wdm],pba->has_wdm,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_wdm],pba->has_wdm,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_pseudo_p_wdm],pba->has_wdm,storeidx);
    /*class_store_double(dataptr,pvecback[pba->index_bg_p_wdm]/pvecback[pba->index_bg_rho_wdm],pba->has_wdm,storeidx);*/
