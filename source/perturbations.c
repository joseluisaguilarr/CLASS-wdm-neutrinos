...
int perturb_init(
  ...
  if (pba->has_wdm == _TRUE_) {
	  printf("Computing WDM perturbations with a_nr = %g \n",pba->bdm_ac);
  }
...
int perturb_indices_of_perturbs(
  ...
  /* JL */
  ppt->has_source_delta_wdm = _FALSE_;
  ...
  /* JL */
  ppt->has_source_theta_wdm = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        /* JL */
        if (pba->has_wdm == _TRUE_)
          ppt->has_source_delta_wdm = _TRUE_;  
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        /* JL */  
        if (pba->has_wdm == _TRUE_)
          ppt->has_source_theta_wdm = _TRUE_;
      }
      ...
      /* JL */
      class_define_index(ppt->index_tp_delta_wdm, ppt->has_source_delta_wdm,index_type,1);
      ...
      /* JL */
      class_define_index(ppt->index_tp_theta_wdm, ppt->has_source_theta_wdm,index_type,1);
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    /* JL 
    if (pba->has_wdm == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, 2);
    */
...
int perturb_solve(
  ...
  /* JL */
  if (pba->has_wdm == _TRUE_) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_wdm]/ppw->pvecback[pba->index_bg_rho_wdm]-1./3.)>ppr->tol_wdm_initial_w,
                 ppt->error_message,
                 "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time at which the BDM is not ultra-relativistic anymore, with w=%g, p=%g and rho=%g\n",
                 ppw->pvecback[pba->index_bg_p_wdm]/ppw->pvecback[pba->index_bg_rho_wdm],
                 ppw->pvecback[pba->index_bg_p_wdm],
                 ppw->pvecback[pba->index_bg_rho_wdm]);
  }
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    /* JL */
    if (pba->has_wdm == _TRUE_) {
        if (fabs(ppw->pvecback[pba->index_bg_p_wdm]/ppw->pvecback[pba->index_bg_rho_wdm]-1./3.) > ppr->tol_wdm_initial_w)
          is_early_enough = _FALSE_;
    }
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* JL */
      if ((pba->has_wdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
	  
          class_store_columntitle(ppt->scalar_titles,"delta_wdm",_TRUE_);
          class_store_columntitle(ppt->scalar_titles,"theta_wdm",_TRUE_);
          class_store_columntitle(ppt->scalar_titles,"shear_wdm",_TRUE_);
          class_store_columntitle(ppt->scalar_titles,"cs2_wdm",_TRUE_);
      }
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    /* JL */
      if (pba->has_wdm == _TRUE_) { 
      class_define_index(ppv->index_pt_delta_wdm,pba->has_wdm,index_pt,1); 
      class_define_index(ppv->index_pt_theta_wdm,pba->has_wdm,index_pt,1); 
      class_define_index(ppv->index_pt_shear_wdm,pba->has_wdm,index_pt,1);
    }        
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      /* JL */
      if (pba->has_wdm == _TRUE_) {
        ppv->y[ppv->index_pt_delta_wdm] =
               ppw->pv->y[ppw->pv->index_pt_delta_wdm];
        ppv->y[ppv->index_pt_theta_wdm] =
               ppw->pv->y[ppw->pv->index_pt_theta_wdm];
        ppv->y[ppv->index_pt_shear_wdm] =
               ppw->pv->y[ppw->pv->index_pt_shear_wdm];
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    /* JL */
    if (pba->has_wdm == _TRUE_) {
        rho_r += ppw->pvecback[pba->index_bg_rho_wdm];
        rho_nu += ppw->pvecback[pba->index_bg_rho_wdm];
    }
    ...
    */
    ...
    if (pba->has_wdm == _TRUE_) {
	  ...
	  f_wdm = pow(pow(a/pba->a_today,2)/pba->H0,2)*ppw->pvecback[pba->index_bg_rho_wdm];          

      ppw->pv->y[ppw->pv->index_pt_delta_wdm] = delta_ur * f_wdm;

      ppw->pv->y[ppw->pv->index_pt_theta_wdm] =  4./(3.*k) * theta_ur * f_wdm;

      ppw->pv->y[ppw->pv->index_pt_shear_wdm] = 2. * shear_ur * f_wdm;

      // pfpw->pv->y[ppw->pv->index_pt_l3_ncdm] = l3_ur * f_wdm;
...
int perturb_total_stress_energy(
  ...
  double theta_wdm=0.;
  double shear_wdm=0.;
  double rho_delta_wdm=0.;
  double rho_plus_p_theta_wdm=0.;
  double rho_plus_p_shear_wdm=0.;
  double delta_p_wdm=0.;
  ...
  /* JL */
  double rho_plus_p_wdm;
  double cg2_wdm,w_wdm,rho_wdm_bg,p_wdm_bg,pseudo_p_wdm;
  ...
  if (_scalars_) {
    ...
    if (pba->has_wdm == _TRUE_) {
      // The perturbations are evolved integrated: fluid approx
      rho_wdm_bg = ppw->pvecback[pba->index_bg_rho_wdm];
      p_wdm_bg = ppw->pvecback[pba->index_bg_p_wdm];
      pseudo_p_wdm = ppw->pvecback[pba->index_bg_pseudo_p_wdm];

      rho_plus_p_wdm = rho_wdm_bg + p_wdm_bg;
      w_wdm = p_wdm_bg/rho_wdm_bg;
      cg2_wdm = w_wdm*(1.0-1.0/(3.0+3.0*w_wdm)*(3.0*w_wdm-2.0+pseudo_p_wdm/p_wdm_bg));
	  
	  // testing
	  //if (a > pba->bdm_ac)
  	  //  printf(">>> a=%g; ac=%g; w =%g=%g; cg2=%g \n", a, pba->bdm_ac, p_wdm_bg/rho_wdm_bg, pseudo_p_wdm/p_wdm_bg/3., cg2_wdm);
	  
      if ((ppt->has_source_delta_wdm == _TRUE_) || (ppt->has_source_theta_wdm == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
        delta_wdm = y[ppw->pv->index_pt_delta_wdm];
        theta_wdm = y[ppw->pv->index_pt_theta_wdm];
	shear_wdm = y[ppw->pv->index_pt_shear_wdm];
      }

      ppw->delta_rho += rho_wdm_bg*delta_wdm;
      ppw->rho_plus_p_theta += rho_plus_p_wdm*y[ppw->pv->index_pt_theta_wdm];
      ppw->rho_plus_p_shear += rho_plus_p_wdm*y[ppw->pv->index_pt_shear_wdm];
      ppw->delta_p += cg2_wdm*rho_wdm_bg*delta_wdm;
      rho_plus_p_tot += rho_plus_p_wdm;      
    }
    ...
    */
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      /* JL */
      if (pba->has_wdm == _TRUE_) {
          delta_rho_m +=  (ppw->pvecback[pba->index_bg_rho_wdm] - 3.* ppw->pvecback[pba->index_bg_p_wdm])*delta_wdm;
      /*Corregi esta parte*/    
          rho_m += ppw->pvecback[pba->index_bg_rho_wdm] - 3.* ppw->pvecback[pba->index_bg_p_wdm];
      }
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
      ...
      /* JL */
      if (pba->has_wdm == _TRUE_) {
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_wdm]+ppw->pvecback[pba->index_bg_p_wdm])* theta_wdm;
          rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_wdm]+ppw->pvecback[pba->index_bg_p_wdm]);
      }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_wdm == _TRUE_) {
      _set_source_(ppt->index_tp_delta_wdm) = y[ppw->pv->index_pt_delta_wdm];
    }
    ...
    /* JL */
    /* theta_wdm */
    if (ppt->has_source_theta_wdm == _TRUE_) {
        _set_source_(ppt->index_tp_theta_wdm) = y[ppw->pv->index_pt_theta_wdm];
    }
  }
...
int perturb_print_variables(double tau,
  ...
  /** - ncdm sector ends */
  /** JL */
  double delta_wdm=0., theta_wdm=0., shear_wdm=0., delta_p_over_delta_rho_wdm=0.;
  double rho_wdm_bg, p_wdm_bg, pseudo_p_wdm, w_wdm;
  double rho_delta_wdm = 0.0;
  double rho_plus_p_theta_wdm = 0.0;
  double rho_plus_p_shear_wdm = 0.0;
  double delta_p_wdm = 0.0;
  ...
  if (_scalars_) {
    ...
    if (pba->has_wdm == _TRUE_) {
      /** - --> Get delta, deltaP/rho, theta, shear and store in array */
        // The perturbations are as fluid approx.
          rho_wdm_bg = pvecback[pba->index_bg_rho_wdm];
          p_wdm_bg = pvecback[pba->index_bg_p_wdm];
          pseudo_p_wdm = pvecback[pba->index_bg_pseudo_p_wdm];
          w_wdm = p_wdm_bg/rho_wdm_bg;
          ...
          shear_wdm = y[ppw->pv->index_pt_shear_wdm];
          //This is the adiabatic sound speed:
          delta_p_over_delta_rho_wdm = w_wdm*(1.0-1.0/(3.0+3.0*w_wdm)*(3.0*w_wdm-2.0+pseudo_p_wdm/p_wdm_bg));
    }
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* JL */
      if (pba->has_wdm == _TRUE_) {
	  w_wdm = pvecback[pba->index_bg_p_wdm]/ pvecback[pba->index_bg_rho_wdm];
          delta_wdm -= 3.* (1 + w_wdm) * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
          theta_wdm += k*k*alpha;
      } 
    ...
    /* JL */
    if ((pba->has_wdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
        class_store_double(dataptr, delta_wdm, _TRUE_, storeidx);
        class_store_double(dataptr, theta_wdm, _TRUE_, storeidx);
        class_store_double(dataptr, shear_wdm, _TRUE_, storeidx);
        class_store_double(dataptr, delta_p_over_delta_rho_wdm,  _TRUE_, storeidx);
    }
    /* Decaying cold dark matter */
...
int perturb_derivs(double tau,
  ...
  /* JL */
  /* for use with wdm: */
  double rho_wdm_bg,p_wdm_bg,pseudo_p_wdm,w_wdm,ca2_wdm,ceff2_wdm=0.,cvis2_wdm=0.;
  ...
  if (_scalars_) {
    ...
    if (pba->has_wdm == _TRUE_) {
          /** - -----> define intermediate quantitites */
          rho_wdm_bg = pvecback[pba->index_bg_rho_wdm]; /* background density */
          p_wdm_bg = pvecback[pba->index_bg_p_wdm]; /* background pressure */
          pseudo_p_wdm = pvecback[pba->index_bg_pseudo_p_wdm]; /* pseudo-pressure (see CLASS IV paper) */
          w_wdm = p_wdm_bg/rho_wdm_bg; /* equation of state parameter */
          ca2_wdm = w_wdm/3.0/(1.0+w_wdm)*(5.0-pseudo_p_wdm/p_wdm_bg); /* adiabatic sound speed */ /* REVISAR URGENTE*/
          ...
          /* different ansatz for sound speed c_eff and viscosity speed c_vis */
          if (ppr->wdm_fluid_approximation == wdmfa_mb) {
            ceff2_wdm = ca2_wdm;
            cvis2_wdm = 3.*w_wdm*ca2_wdm;
          }
          if (ppr->wdm_fluid_approximation == wdmfa_hu) {
            ceff2_wdm = ca2_wdm;
            cvis2_wdm = w_wdm;
          }
          if (ppr->wdm_fluid_approximation == wdmfa_CLASS) {
            ceff2_wdm = ca2_wdm;
            cvis2_wdm = 3.*w_wdm*ca2_wdm;
          }
		  ...
		  // Testing
		  //if (a < pba->bdm_ac)
		  //  printf(">>> a=%g; ac=%g; w=%g; ca2=%g; \n",a,pba->bdm_ac, w_wdm, ca2_wdm);

          /** - -----> exact continuity equation */

          dy[pv->index_pt_delta_wdm] = -(1.0+w_wdm)*(y[pv->index_pt_theta_wdm]+metric_continuity)-
            3.0*a_prime_over_a*(ceff2_wdm-w_wdm)*y[pv->index_pt_delta_wdm];

          /** - -----> exact euler equation */

          dy[pv->index_pt_theta_wdm] = -a_prime_over_a*(1.0-3.0*ca2_wdm)*y[pv->index_pt_theta_wdm]+
            ceff2_wdm/(1.0+w_wdm)*k2*y[pv->index_pt_delta_wdm]-k2*y[pv->index_pt_shear_wdm]
            + metric_euler;
          ...
          if (ppr->wdm_fluid_approximation == wdmfa_mb) {
            ...
            dy[pv->index_pt_shear_wdm] = -3.0*(a_prime_over_a*(2./3.-ca2_wdm-pseudo_p_wdm/p_wdm_bg/3.)+1./tau)*y[pv->index_pt_shear_wdm]
              +8.0/3.0*cvis2_wdm/(1.0+w_wdm)*s_l[2]*(y[pv->index_pt_theta_wdm]+metric_shear);

          }

          if (ppr->wdm_fluid_approximation == wdmfa_hu) {

            dy[pv->index_pt_shear_wdm] = -3.0*a_prime_over_a*ca2_wdm/w_wdm*y[pv->index_pt_shear_wdm]
              +8.0/3.0*cvis2_wdm/(1.0+w_wdm)*s_l[2]*(y[pv->index_pt_theta_wdm]+metric_shear);
          ...
          if (ppr->wdm_fluid_approximation == wdmfa_CLASS) {
            ...
            dy[pv->index_pt_shear_wdm] = -3.0*(a_prime_over_a*(2./3.-ca2_wdm-pseudo_p_wdm/p_wdm_bg/3.)+1./tau)*y[pv->index_pt_shear_wdm]
              +8.0/3.0*cvis2_wdm/(1.0+w_wdm)*s_l[2]*(y[pv->index_pt_theta_wdm]+metric_ufa_class);
