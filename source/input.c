...
int input_read_parameters(
  ...
  /* JL */
  class_call(parser_read_double(pfc,"Omega_wdm",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_wdm",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
            
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_wdm or omega_wdm, choose one");
  if (flag1 == _TRUE_) {
    pba->Omega0_wdm = param1;
  }
  if (flag2 == _TRUE_){
    pba->Omega0_wdm = param2/pba->h/pba->h;
  }
  
  printf("x. %e \n",pba->Omega0_wdm);

  Omega_tot += pba->Omega0_wdm;
  
  class_call(parser_read_double(pfc,"bdm_ac",&param1,&flag1,errmsg),
             errmsg,
             errmsg); 
  ...
  */
  ...
  /* JL */
  class_read_double("tol_wdm_initial_w",ppr->tol_wdm_initial_w);
  ...
  /* JL */
  class_read_int("wdm_fluid_approximation",ppr->wdm_fluid_approximation);
...
int input_default_precision ( struct precision * ppr ) {
  ...
  /* JL */
  ppr->wdm_fluid_approximation = wdmfa_CLASS;
